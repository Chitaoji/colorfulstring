...
Available shortcut color properties are `d/r/g/y/b/p/c/w`
(dark/red/green/yellow/blue/purple/cyan/white).

You can chain them for background presets, e.g. `c.b.g` means blue foreground + green
background.
...
Use `.faint` to switch the foreground to a faint ANSI variant. It must follow the
...
Besides fluent chaining, `colorfulstring` can also parse inline token fragments from
plain strings:
